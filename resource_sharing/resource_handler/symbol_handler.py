...
class SymbolResourceHandler(BaseResourceHandler, SymbolResolverMixin):
    ...
    def _get_child_groups_tags_ids(self):
        ...
        try:
            # QGIS 2
            return [
                self.style.groupId(n)
                for n in self.style.childGroupNames(parent_group_name)
            ]
        except AttributeError:
            # not QGIS 2, so hopefully QGIS 3
            return [
                self.style.tagId(tag)
    ...
    def _get_symbols_for_group_or_tag(self, symbol_type, child_group_or_tag_id):
        ...
        try:
            # QGIS 2
            return self.style.symbolsOfGroup(symbol_type, child_group_or_tag_id)
        except AttributeError:
            # not QGIS 2, so hopefully QGIS 3
            return self.style.symbolsWithTag(symbol_type, child_group_or_tag_id)
    ...
    def _group_or_tag(self, symbol_type, symbol_name, tag_or_group):
        ...
        try:
            ...
            # not QGIS 2, so hopefully QGIS 3
            self.style.tagSymbol(
                symbol_type, symbol_name, [self.style.tag(tag_or_group)]
            )
        ...
        for symbol_file in symbol_files:
            ...
            for symbol in symbol_xml_extractor.symbols:
                ...
                if self.style.addSymbol(symbol_name, symbol[SYMBOL], True):
                    self._group_or_tag(
                        QgsStyle.SymbolEntity, symbol_name, groupOrTag_id
                    )
            ...
            for textformat in symbol_xml_extractor.textformats:
                ...
                if self.style.addTextFormat(
                    ...
                    self._group_or_tag(
                        QgsStyle.TextFormatEntity, textformat_name, groupOrTag_id
                    )
            for labelsetting in symbol_xml_extractor.labelsettings:
                ...
                if self.style.addLabelSettings(
                    ...
                    self._group_or_tag(
                        QgsStyle.LabelSettingsEntity, labelsetting_name, groupOrTag_id
                    )
        if valid >= 0:
