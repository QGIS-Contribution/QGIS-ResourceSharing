...
class CustomSortFilterProxyModel(QSortFilterProxyModel):
    """Cuustom QSortFilterProxyModel to be able to search on multiple data."""
    ...
    def filterAcceptsRow(self, row_num, source_parent):
        ...
        name = self.filterRegExp().indexIn(
            self.sourceModel().data(
                index, COLLECTION_NAME_ROLE).encode('utf-8')) >= 0
        author = self.filterRegExp().indexIn(
            self.sourceModel().data(
                index, COLLECTION_AUTHOR_ROLE).encode('utf-8')) >= 0
        description = self.filterRegExp().indexIn(
            self.sourceModel().data(
                index, COLLECTION_DESCRIPTION_ROLE).encode('utf-8')) >= 0
        tags = self.filterRegExp().indexIn(
            self.sourceModel().data(
                index, COLLECTION_TAGS_ROLE).encode('utf-8')) >= 0
